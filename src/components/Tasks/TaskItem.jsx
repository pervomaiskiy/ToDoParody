...
export default function TaskItem({task,id,icon,deleteTask,completedTask,renameTaskName}) {
    
    const [taskRename,setTaskRename] = useState()

    const editTaskName =()=>{
        const newTaskName = window.prompt('Title name',task.name)

        if(newTaskName){
            renameTaskName(id,newTaskName)

        }
    }
    return (
        <div key={id} className={'tasks_item'}>
            <div className={'tasks_items-row'}>
                <div className={'checkbox'}>
                    <input id={id} type={'checkbox'} checked={task.done}></input>
                    <label htmlFor={id}
